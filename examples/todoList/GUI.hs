...
type instance RecRenderOutVal (t :: Todo) = ActionOutput t
...
renderSt :: RenderSt Todo (AllState TodoList (ActionStMap Todo))
renderSt sst resource@(AllState (TodoList _ entMap) otherSt) tchan w = case sst of
  SMain -> do
  ...
  SAction saction sfrom -> do
    ...
    Just (actPage, uiVal) <- liftIO $ runUI w $ renderSt saction resource tchan w
...
main = do
  startGUI
    ...
    ( \w -> do
        ...
        nsRef <- liftIO $ newInternalStRef SMain $ AllState state D.empty
        getHead w
          #+ [ UI.link
